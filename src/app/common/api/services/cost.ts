...
export class Cost {
  ...
  public getCosts() {
    return this.http.get<CostResponse[]>(Endpoints.FIXED_COST);
  }

  public getCostById(id_cost: string) {
    return this.http.get<CostResponse>(`${Endpoints.FIXED_COST}/${id_cost}`);
  }
  ...
  public deleteCost(id_cost: number) {
    return this.http.delete<void>(`${Endpoints.FIXED_COST}/${id_cost}`);
  }
}
